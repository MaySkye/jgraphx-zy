...
import com.mxgraph.examples.swing.util.mxcellUtil;
        ...
        if (graphComponent.isEnabled() && isEnabled() && !e.isConsumed()
            ...
            if (cell != null) {
                ...
                if (isMoveEnabled() && !e.isPopupTrigger()) {
                    ...
                    this.cellCh = (mxCell) obj;
                    this.cell_type = this.cellCh.getType();
                    this.cell_name = this.cellCh.getName();
                    System.out.println("type:" + cell_type + "   name:" + cell_name);
...
//		mxCell selectionCell = (mxCell) graphComponent.getGraph().getSelectionCell();
//		System.out.println(selectionCell.getType());  // 获取类型
//		System.out.println(selectionCell.getGeometry().getSourcePoint());  // 获取矢量线段起点
//		System.out.println(selectionCell.getGeometry().getTargetPoint());  // 获取矢量线段重点
//		System.out.println(selectionCell.getGeometry().getPoints());  // 获取转折点，当适量线段成对称形状时，值为空；当适量线段不规则会出现转折关键点坐标
        ...
        if (graphComponent.isEnabled() && isEnabled() && !e.isConsumed()) {
            ...
            if (first == null
                ...
                if (graph.isSplitEnabled()
                    ...
                    if (cell_name != null) {
						/*if(cell_type.equals("FiberEdge")){
							//dx,dy代表坐标的变化
							mxPoint edge_source= cellCh.getGeometry().getSourcePoint();
							mxPoint edge_target= cellCh.getGeometry().getTargetPoint();
							System.out.println("edge_source:"+edge_source.getX()+"||"+edge_source.getY());
							System.out.println("edge_target:"+edge_target.getX()+"||"+edge_target.getY());
							coordinateClass source_coor=new coordinateClass((int) edge_source.getX(),(int) edge_source.getY());
							coordinateClass target_coor=new coordinateClass((int) edge_target.getX(),(int) edge_target.getY());

							int middleX= (int) ((edge_source.getX()+edge_target.getX())/2);
							int middleY= (int) ((edge_source.getY()+edge_target.getY())/2);

							coordinateClass source_target_coor=getTargetCoor(edge_source,true,middleX,middleY);
							coordinateClass targrt_targrt_coor=getTargetCoor(edge_target,false,middleX,middleY);
							coordinateClass result_coor=null;

							if(source_target_coor!=null&&targrt_targrt_coor!=null){
								//求target坐标距离连接线source/target的距离远近
								int dis_source_target=getdistance(source_coor,source_target_coor);
								int dis_targrt_targrt=getdistance(target_coor,targrt_targrt_coor);
								//选择距离最小的坐标
								if(dis_source_target>=dis_targrt_targrt){
									result_coor=targrt_targrt_coor;
								}else{
									result_coor=source_target_coor;
								}

							}

							if(result_coor!=null){
								moveCells(cells, result_coor.getX(), result_coor.getY(), target, e);
								System.out.println(result_coor.getX()+" "+result_coor.getY()+"||"+dx+" "+dy);
							} else{
        ...
        WWFiberManager.doHandleForMxCells(graphComponent.getGraph().getSelectionCells(), graphComponent.getGraph());
    }

    protected coordinateClass getTargetCoor(mxPoint point, boolean isSourcePoint, int middleX, int middleY) {

        //List<mxCell> ports_list=new LinkedList<>();
        Set<mxCell> set = new HashSet<>();
        Set<mxCell> port_set = new HashSet<>();
        coordinateClass result_coor = null;
        mxCell fit_cell = null; //代表符合条件的图元
        //mxCell fit_port=null; //代表符合条件的port，fit_port为fit_cell的子孩子
        int start_x = (int) point.getX();
        int start_y = (int) point.getY();
        coordinateClass point_coor = new coordinateClass(start_x, start_y);

        int distance = 0;
        int times = 0;
        int temp = 0;
        coordinateClass cell_coor = null;

        mxCell mcellLU, mcellRU, mcellLD, mcellRD, mcellU, mcellD, mcellL, mcellR;

        //判断规定范围内是否存在图元
        System.out.println("start_x:" + start_x + "  start_y:" + start_y + " middleX:" + middleX + "  middleY:" + middleY);
        for (int i = 1; i < 70; i++) {
		    /*if(start_x+i>middleX||start_y-i<middleY){
				System.out.println("不符合条件");
				break;
			}*/
            set.clear();
            port_set.clear();
            //得到了八个方向的图元,以start_x，start_y为圆心
            mcellLU = (mxCell) graphComponent.getCellAt(start_x - i, start_y - i);
            mcellRU = (mxCell) graphComponent.getCellAt(start_x + i, start_y - i);
            mcellLD = (mxCell) graphComponent.getCellAt(start_x - i, start_y + i);
            mcellRD = (mxCell) graphComponent.getCellAt(start_x + i, start_y + i);
            mcellU = (mxCell) graphComponent.getCellAt(start_x, start_y - i);
            mcellD = (mxCell) graphComponent.getCellAt(start_x, start_y + i);
            mcellL = (mxCell) graphComponent.getCellAt(start_x - i, start_y);
            mcellR = (mxCell) graphComponent.getCellAt(start_x + i, start_y);
            ...
            set.add(mcellL);
            set.add(mcellR);
            //比较八个图元，选择出最符合条件，距离最近的一个
            //先比较type,优先选择type==Port，然后比较距离
            for (mxCell cell : set) {
                if (cell != null && cell.getType().equals("Port")) {
                    port_set.add(cell);
                }
                if (cell != null && cell.getType().equals("FiberEdge")) {
                    set.remove(cell);
                }
                if (cell == null) {
                    set.remove(cell);
            ...
            if (port_set.size() > 0) {
                System.out.println("port_set is not null");
                System.out.println("port_set.size():" + port_set.size());
                for (mxCell cell : port_set) {
                    if (times == 0) {
                        cell_coor = mxcellUtil.getPortCoor((mxCell) cell.getParent(), cell.getLocation(), cell.getDirection());
                        distance = getdistance(point_coor, cell_coor);
                        fit_cell = cell;
                        result_coor = cell_coor;
                        times = 1;
                    } else {
                        cell_coor = mxcellUtil.getPortCoor((mxCell) cell.getParent(), cell.getLocation(), cell.getDirection());
                        temp = getdistance(point_coor, cell_coor);
                        if (temp < distance) {
                            distance = temp;
                            fit_cell = cell;
                            result_coor = cell_coor;
                ...
                for (mxCell cell : set) {
                    if (times == 0) {
                        cell_coor = new coordinateClass((int) cell.getGeometry().getX(), (int) cell.getGeometry().getY());
                        distance = getdistance(point_coor, cell_coor);
                        fit_cell = cell;
                        result_coor = cell_coor;
                        times = 1;
			...
			/*if(cellLU != null &&!mcellLU.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellLU.getType());
				System.out.println("mcellLU  x:"+mcellLU.getGeometry().getX()+"  y:"+mcellLU.getGeometry().getY());
				fit_cell = mcellLU;
			} else if (cellRU != null&&!mcellRU.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellRU.getType());
				System.out.println("mcellRU  x:"+mcellRU.getGeometry().getX()+"  y:"+mcellRU.getGeometry().getY());
				fit_cell = mcellRU;
			} else if (cellLD != null&&!mcellLD.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellLD.getType());
				System.out.println("mcellLD  x:"+mcellLD.getGeometry().getX()+"  y:"+mcellLD.getGeometry().getY());
				fit_cell = mcellLD;
			} else if (cellRD != null && !mcellRD.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellRD.getType());
				System.out.println("mcellRD  x:"+mcellRD.getGeometry().getX()+"  y:"+mcellRD.getGeometry().getY());
				fit_cell = mcellRD;
			} else if (cellR != null && !mcellR.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellR.getType());
				System.out.println("mcellR  x:"+mcellR.getGeometry().getX()+"  y:"+mcellR.getGeometry().getY());
				fit_cell = mcellR;
			} else if (cellL != null && !mcellL.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellL.getType());
				System.out.println("mcellL  x:"+mcellL.getGeometry().getX()+"  y:"+mcellL.getGeometry().getY());
				fit_cell = mcellL;
			} else if (cellU != null && !mcellU.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellU.getType());
				System.out.println("mcellU  x:"+mcellU.getGeometry().getX()+"  y:"+mcellU.getGeometry().getY());
				fit_cell = mcellU;
			} else if (cellD != null && !mcellD.getType().equals("FiberEdge")){
				System.out.println("type:"+mcellD.getType());
				System.out.println("mcellD  x:"+mcellD.getGeometry().getX()+"  y:"+mcellD.getGeometry().getY());
				fit_cell = mcellD;
			}*/
        }
    ...
    protected int getdistance(coordinateClass coor1, coordinateClass coor2) {
        int x = (coor1.getX() - coor2.getX()) * (coor1.getX() - coor2.getX());
        int y = (coor1.getY() - coor2.getY()) * (coor1.getY() - coor2.getY());

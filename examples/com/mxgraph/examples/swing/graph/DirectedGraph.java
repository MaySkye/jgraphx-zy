 ...
 * @Author:zhoayi
 * @Description:
 * @Data: Created in 10:16 2018/8/7
 * @Modify By:
 */
public class DirectedGraph<T> implements GraphInterface<T>,java.io.Serializable{
    ...
    private Map<String, MutableTriple<VertexInterface<T>,EdgeLink,VertexInterface<T>>> edges; //String为边的唯一标识
    ...
    public void printVertices(Map<String, VertexInterface<String>> vertexs){
        Iterator<Map.Entry<String, VertexInterface<String>>> v_entries = vertexs.entrySet().iterator();
        while (v_entries.hasNext()) {
            Map.Entry<String, VertexInterface<String>> entry = v_entries.next();
            VertexInterface<String> v=entry.getValue();
            System.out.println("顶点："+v.getLabel());
        }
    }

    public void printEdges(Map<String, MutableTriple<VertexInterface<String>,EdgeLink,VertexInterface<String>>> edges){
        Iterator<Map.Entry<String, MutableTriple<VertexInterface<String>,EdgeLink,VertexInterface<String>>>> e_entries = edges.entrySet().iterator();
        while (e_entries.hasNext()) {
            Map.Entry<String, MutableTriple<VertexInterface<String>,EdgeLink,VertexInterface<String>>> entry = e_entries.next();
            MutableTriple<VertexInterface<String>,EdgeLink,VertexInterface<String>> e=entry.getValue();
            System.out.println("边："+e.getMiddle().getId());
        }
    }
    @Override
    ...
    @Override
    public void addV(VertexInterface<T> v) {
        vertices.put(v.getLabel(), v);
    }

    @Override
    public boolean addEdge(T begin, T end, EdgeLink edgelink) {
        ...
        if(beginVertex != null && endVertex != null)
            result = beginVertex.connect(endVertex, edgelink);//起始点与终点连接,即成一条边
       /* if(beginVertex.getState().equals("show")&&endVertex.getState().equals("show")){
            edgelink.setState("show");
        }*/
        if(result)
            edgeCount++;
        MutableTriple mt=new MutableTriple<>(beginVertex,edgelink,endVertex);

        edges.put(String.valueOf(edgeCount),mt);

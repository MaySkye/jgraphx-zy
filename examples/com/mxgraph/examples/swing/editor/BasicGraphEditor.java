...
public class BasicGraphEditor extends JPanel {
    ...
    private static int MonitorNameWidth = 150;  // 字段域宽度
    private static int MonitorDataWidth = 120;  // 值域宽度
    private static int MonitorUnitWidth = 40;  // 单位域宽度
    private static int DeviceWidth = 310;  // 设备id域宽度
    private static int DeviceHeight = 30;  // 设备id域高度
            ...
            if (child.isVertex() && child.getV() != null) {
                ...
                if (child.getV().getName() != null) {
                    ...
                    mxCell nameCell = new mxCell(child.getV().getId(), child.getV().getId(),
                            new mxGeometry(x  - (DeviceWidth - imageSize) / 2, y, DeviceWidth, DeviceHeight), nameStyle);
                    ...
                    for (int r = 0; r < arr.length; r++) {
                        ...
                        if (!mxcell_property.containsKey(arr[r].trim())) {
                            ...
                            mxCell nameMonitorCell = new mxCell(arr[r].trim(), owlObject.type.id,
                                    new mxGeometry(x  - (DeviceWidth - imageSize) / 2, (y + MonitorHeight * r + 30), MonitorNameWidth, MonitorHeight), nameStyle);
                            ...
                            if (!unit.equals("")) {
                                dataMonitorCell = new mxCell(arr[r].trim(), "",
                                        new mxGeometry(x + MonitorNameWidth  - (DeviceWidth - imageSize) / 2, (y + MonitorHeight * r + 30), MonitorDataWidth, MonitorHeight), dataStyle);
                            } else {
                                dataMonitorCell = new mxCell(arr[r].trim(), "",
                                        new mxGeometry(x + MonitorNameWidth  - (DeviceWidth - imageSize) / 2, (y + MonitorHeight * r + 30), MonitorDataWidth + MonitorUnitWidth, MonitorHeight), dataStyle);
                            }
                            ...
                            if (!unit.equals("")) {
                                ...
                                mxCell unitMonitorCell = new mxCell(arr[r].trim(), unit,
                                        new mxGeometry(x + MonitorNameWidth + MonitorDataWidth  - (DeviceWidth - imageSize) / 2, (y + MonitorHeight * r + 30),
